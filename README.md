...
**GOAI Virtual Yeast Model V5** 是一个面向 [GOAI「前沿探索 AI for Research」虚拟细胞赛道](https://goaihz.com) 的酿酒酵母蛋白质组预测项目。给定菌株、化合物扰动、时间、温度和测量上下文，模型输出完整的 5,243 维 log2 蛋白丰度向量，并在**未见化合物 (S1)、未见菌株 (S2) 和双重未知 (S3)** 条件下进行可靠外推。
